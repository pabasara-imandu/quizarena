...
`/admin` signs in with Google. Super admins (ලොකු Admin) are listed in `admins.json`; they
add and remove normal admins (පොඩි admin) from the page. Both get the fleet view and the
**translation editor**: every string in Sinhala, editable, saved for everyone within a
minute. See `DEPLOY.md` and `client/lib/i18n/README.md`.
